...
def loadCameraPara():
    K1 = np.load("camearPara/K1.npy")
    K2 = np.load("camearPara/K2.npy")
    distL = np.load("camearPara/distL.npy")
    distR = np.load("camearPara/distR.npy")
    R = np.load("camearPara/rotation.npy")
    T = np.load("camearPara/translation.npy")
    return K1, K2, distL, distR, R, T

def rectifyImages(K1, K2, distL, distR, R, T, imgL, imgR):
    weight_height = imgL.shape[::-1]
    rectL, rectR, projL, projR, _, _, _ = cv2.stereoRectify(K1, distL, K2, distR, weight_height, R, T, 1, (0, 0))

    stereoMapL = cv2.initUndistortRectifyMap(K1, distL, rectL, projL, weight_height, cv2.CV_16SC2)
    stereoMapR = cv2.initUndistortRectifyMap(K2, distR, rectR, projR, weight_height, cv2.CV_16SC2)

    rec_L = cv2.remap(imgL, stereoMapL[0], stereoMapL[1], cv2.INTER_AREA)
    rec_R = cv2.remap(imgR, stereoMapR[0], stereoMapR[1], cv2.INTER_AREA)
    return rec_L, rec_R


if __name__ == '__main__':
    # read image pairs
    # please uncomment below lines to
    # compute the disparity of corresponding objects
    # obj = "mouse"
    obj = "bottle"
    # obj = "box"
    imgL = cv2.pyrDown(cv2.imread("mypic/" + obj + "L/imageL1.png", 0))
    # imgL = cv2.GaussianBlur(imgL, (3,3), 0, 0)
    imgR = cv2.pyrDown(cv2.imread("mypic/" + obj + "R/imageR1.png", 0))
    ...
    K1, K2, distL, distR, R, T = loadCameraPara()
    imgL_rectified, imgR_rectified = rectifyImages(K1, K2, distL, distR, R, T, imgL, imgR)
