<?php
	...
	$sql = "SELECT * FROM OrderingInfo WHERE RepName = '$user'";
	...
	if($result->num_rows>0){
		while($row = $result-> fetch_assoc()) {
			...
			if ($status!=='Anomaly'){
			if ($gap>24){
				$update = "UPDATE OrderingInfo SET Status='Complete' WHERE (RepName='$user' AND Time='$pretime')";
				mysqli_query($link,$update);
			}else{
				$update = "UPDATE OrderingInfo SET Status='Processing' WHERE (RepName='$user' AND Time='$pretime')";
				mysqli_query($link,$update);
			}
			}
		}
	...
	$sql = "SELECT * FROM OrderingInfo WHERE RepName = '$user' LIMIT $this_page_first_result,$results_per_pages";
	$result = $link->query($sql);	
	if ($result->num_rows >0){
		while($row = $result-> fetch_assoc()) {
	...
	for($page=1;$page<=$number_of_pages;$page++){
		echo '<a href="reporder.php?page='.$page.'"><input type="button" class="pagebutton" value='.$page.'></a>';
	}
	echo '</div>';
?>
