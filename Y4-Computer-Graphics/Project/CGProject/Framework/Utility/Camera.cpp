...
void Camera::Reset(){
	...
	eyePosition[0] = 0.0f;
	eyePosition[1] = 0.0f;
	eyePosition[2] = 0.5f * static_cast<float>(Scene::GetWindowHeight()) / static_cast<float>(tan(M_PI / 4.0));//0.0f;
	//eyePosition[2] = 10;
...
void Camera::SetupCamera()
{
	...
	gluLookAt(eyePosition[0], eyePosition[1], eyePosition[2],
			  ...
			  up[0], up[1], up[2]);
	GLfloat arr[] = { eyePosition[0], eyePosition[1], eyePosition[2],
		eyePosition[0] + vd[0], eyePosition[1] + vd[1], eyePosition[2] + vd[2],
			  up[0], up[1], up[2] };
	viewMatrix = glm::mat4(glm::make_mat3(arr));
...
void Camera::HandleMouseDrag(int x, int y)
{
	...
	switch (currentButton)
	{
		case GLUT_LEFT_BUTTON:
			...
			/* As we want out camera to stay on the same plane at the same height (i.e. not move up and down the y axis) update a forward direction vector which can be used to move the camera. 
			This forward vector moves the camera in the same direction as the view direction except it will not contain any y component so it cannot move off of its original height. 
